<template>
  <div>
        ...
        <div class="feed-container-content feed-background-color">
          <div>
            ...
            <p 
              ...
              v-html="feedData.content ? feedData.content.replace(/\n/g,'<br/>') : feedData.content"
            >
            </p>
          </div>
        </div>
            ...
            <Row :gutter="24" style="display: flex; align-items: center;">
              ...
              <Col span="9">
                <div class="detail-data">
                  ...
                  <p>{{ friendNum(feedData.views) }}人浏览</p>
                </div>
              </Col>
            </Row>
    ...
    <div id="comments" class="noComment" v-if="!feedData.comments_count">
    ...
    <div id="feed-footer" class="feed-container-tool-operation feed-background-color">
      <Row :gutter="24" style="display: flex; justify-content: center; align-items: center; height: 100%;">
        ...
        <Col span="6" class="operation">
          <div v-if="!feedData.has_collection" @click="handleCollection(feed_id)">
 ...
 export default {
    ...
    methods: {
      ...
      handleCollection (feed_id) {
        addAccessToken().post(createAPI(`groups/${this.group_id}/posts/${this.feed_id}/collection`), {}, {
      ...
      handleUnCollection (feed_id) {
        addAccessToken().delete(createAPI(`groups/${this.group_id}/posts/${this.feed_id}/collection`), {}, {
      ...
      handleDiggFeed (feed_id) {
        addAccessToken().post(createAPI(`groups/${this.group_id}/posts/${this.feed_id}/like`), {}, {
          validateStatus: status => status === 201
        })
        .then(response => {
      ...
      handleUnDiggFeed (feed_id) {
        addAccessToken().delete(createAPI(`groups/${this.group_id}/posts/${this.feed_id}/like`), {}, {
          validateStatus: status => status === 204
        })
        .then(response => {
      ...
      loadBottom() {
        ...
        addAccessToken().get(
          createAPI(`groups/${this.group_id}/posts/${this.feed_id}/comments?after=${this.max_id}&limit=${limit}`),
          {},
          {
      ...
      sendComment () {
        ...
        addAccessToken().post(createAPI(`groups/${this.group_id}/posts/${this.feed_id}/comments`), 
          comment_data,
          {
            validateStatus: status => status === 201
          }
      ...
      deleteComment (close, data) {
        addAccessToken().delete(createAPI(`groups/${this.group_id}/posts/${data.feed_id}/comments/{data.comment_id}`), {}, {
          validateStatus: status => status === 204
        })
        .then(response => {
          this.feedData.feed_comment_count -= 1;
    ...
    created () {
      ...
      let group_id = parseInt(this.$route.params.groupID);
      if ( !feed_id ) {
        this.$store.dispatch(NOTICE, cb => {
          cb({
            text: '发生一些错误',
      ...
      addAccessToken().get(
        createAPI(`groups/${group_id}/posts/${feed_id}`),
        {},
        {
          validateStatus: status => status === 200
        }
      ...
      .then(({ data = {} }) => {
        ...
        addAccessToken().get(
          createAPI(`groups/${group_id}/posts/${feed_id}/comments`),
          {
            limit: 15
          },
          {
        ...
        // 获取点赞的用户
        addAccessToken().get(createAPI(`groups/${group_id}/posts/${feed_id}/likes`), {}, {
          validateStatus: status => status === 200
        })
        .then( ({ data = {} }) => {
