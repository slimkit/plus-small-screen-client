  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        if(pinned && (feed_content.length <= this.$store.state.CONFIG.feed.limit)) {
        ...
        this.$http.post(
          'feeds',
          {
            feed_content,
            feed_from: 2,
            feed_mark: (new Date()).valueOf() + '' + this.$store.state.CURRENTUSER.id
          },
          {
    ...
    computed: {
      ...
      tips() {
        return `注: 超过${this.$store.state.CONFIG.feed.limit || 50}字部分内容收费`
      },
      items() {
